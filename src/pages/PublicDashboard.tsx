...
export function PublicDashboard() {
  ...
  const calculateTopContributors = () => {
    ...
    const userIssueCount: { [userId: string]: { count: number; userName: string } } = {};
    
    issues.forEach(issue => {
      if (issue.user_id && !issue.is_anonymous) {
        if (!userIssueCount[issue.user_id]) {
        ...
        userIssueCount[issue.user_id].count++;
      }
    });
    
    // Convert to array and sort by count
    const contributors = Object.entries(userIssueCount)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 flex-grow">
      ...
      {topContributors.length > 0 && (
        <div className="bg-white border-t border-gray-100 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-yellow-50/50 to-amber-50/50"></div>
                ...
                <div className="order-2 md:order-1 w-full md:w-auto mb-6 md:mb-0 animate-fade-in" style={{animationDelay: '0.2s'}}>
                  <div className="relative bg-white rounded-2xl shadow-xl border-2 border-gray-200 p-8 text-center transform hover:scale-105 transition-all duration-300">
                    ...
                    <div className="pt-8">
                      ...
                      <div className="text-sm text-gray-600">Silver Contributor</div>
                      <div className="mt-4 text-4xl font-bold text-gray-600">{topContributors[1].count} <span className="text-sm font-normal">issues</span></div>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="order-1 md:order-2 w-full md:w-auto mb-6 md:mb-0 animate-fade-in">
                  <div className="relative bg-gradient-to-br from-yellow-50 to-amber-50 rounded-3xl shadow-2xl border-4 border-yellow-400 p-10 text-center transform hover:scale-110 transition-all duration-300">
                    ...
                    <div className="pt-12">
                      ...
                      <div className="mt-6 text-5xl font-bold text-yellow-600">{topContributors[0].count} <span className="text-xl font-normal">issues</span></div>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="order-3 w-full md:w-auto mb-6 md:mb-0 animate-fade-in" style={{animationDelay: '0.4s'}}>
                  <div className="relative bg-white rounded-2xl shadow-xl border-2 border-gray-200 p-8 text-center transform hover:scale-105 transition-all duration-300">
                    ...
                    <div className="pt-8">
                      ...
                      <div className="text-sm text-gray-600">Bronze Contributor</div>
                      <div className="mt-4 text-4xl font-bold text-gray-600">{topContributors[2].count} <span className="text-sm font-normal">issues</span></div>
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="card-enhanced p-3 text-center group hover:shadow-custom-lg transition-all duration-300 bg-white rounded-xl">
            <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-primary-100 to-primary-200 rounded-lg mx-auto mb-1 group-hover:scale-110 transition-transform duration-300">
              <AlertTriangle className="w-5 h-5 text-primary-600" />
            </div>
            <div className="text-2xl font-heading font-bold text-gray-900 mb-0.5 animate-pulse-slow">{stats.total}</div>
            <div className="text-xs font-semibold text-gray-600 uppercase">Total Issues</div>
          </div>
          
          <div className="card-enhanced p-3 text-center group hover:shadow-custom-lg transition-all duration-300 bg-white rounded-xl">
            <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-red-100 to-red-200 rounded-lg mx-auto mb-1 group-hover:scale-110 transition-transform duration-300">
              <AlertTriangle className="w-5 h-5 text-red-600" />
            </div>
            <div className="text-2xl font-heading font-bold text-red-600 mb-0.5 animate-pulse-slow">{stats.pending}</div>
            <div className="text-xs font-semibold text-gray-600 uppercase">Pending</div>
          </div>
          
          <div className="card-enhanced p-3 text-center group hover:shadow-custom-lg transition-all duration-300 bg-white rounded-xl">
            <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-accent-100 to-accent-200 rounded-lg mx-auto mb-1 group-hover:scale-110 transition-transform duration-300">
              <AlertTriangle className="w-5 h-5 text-accent-600" />
            </div>
            <div className="text-2xl font-heading font-bold text-accent-600 mb-0.5 animate-pulse-slow">{stats.inProgress}</div>
            <div className="text-xs font-semibold text-gray-600 uppercase">In Progress</div>
          </div>
          
          <div className="card-enhanced p-3 text-center group hover:shadow-custom-lg transition-all duration-300 bg-white rounded-xl">
            <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-secondary-100 to-secondary-200 rounded-lg mx-auto mb-1 group-hover:scale-110 transition-transform duration-300">
              <CheckCircle className="w-5 h-5 text-secondary-600" />
            </div>
            <div className="text-2xl font-heading font-bold text-secondary-600 mb-0.5 animate-pulse-slow">{stats.resolved}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="card-enhanced p-3 text-center group hover:shadow-custom-lg transition-all duration-300 bg-white rounded-xl">
            <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-purple-100 to-purple-200 rounded-lg mx-auto mb-1 group-hover:scale-110 transition-transform duration-300">
              <CheckCircle className="w-5 h-5 text-purple-600" />
            </div>
            <div className="text-2xl font-heading font-bold text-purple-600 mb-0.5 animate-pulse-slow">{stats.resolutionRate}%</div>
            <div className="text-xs font-semibold text-gray-600 uppercase">Resolution Rate</div>
          </div>
          
          <div className="card-enhanced p-3 text-center group hover:shadow-custom-lg transition-all duration-300 bg-white rounded-xl">
            <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-indigo-100 to-indigo-200 rounded-lg mx-auto mb-1 group-hover:scale-110 transition-transform duration-300">
              <AlertTriangle className="w-5 h-5 text-indigo-600" />
            </div>
            <div className="text-2xl font-heading font-bold text-indigo-600 mb-0.5 animate-pulse-slow">{stats.recentActivity}</div>
            <div className="text-xs font-semibold text-gray-600 uppercase">Issues This Week</div>
          </div>
        </div>
        ...
        <div className="card-enhanced p-8 mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              >
                {/* Overlay for better text readability */}
                <div className="absolute inset-0 bg-gradient-to-br from-white/90 to-white/70 group-hover:from-white/85 group-hover:to-white/60 transition-all duration-300"></div>
                ...
                <div className="relative z-10">
                  ...
                  <span className={`font-semibold text-gray-700 group-hover:${item.textColor} transition-colors relative z-10`}>{item.name}</span>
                </div>
              </div>
            ))}
          </div>
